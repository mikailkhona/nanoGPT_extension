...
import hydra
from utils import get_dataloader, get_dataloader_lol, dotdict, get_cosine_warmp_lr, check_generated_path_accuracy, make_prompt
import random
import ipdb
...
def main(cfg):
    ...
    eval_pairs_dict = scm_dict['eval_pairs_dict'].item()
    ...
    while True:
        ...
        if iter_num % cfg.eval_interval == 0:# and master_process:
            ...
            if cfg.deploy:
                ...
                layer_diffs = ['5', '6', '7', '8', '9']
                generated_paths_conditioned_on_length = {}
                edge_accuracies_conditioned_on_length = {}
                does_end_at_targets_conditioned_on_length = {}
                path_lengths_conditioned_on_length = {}
                for layer_diff in layer_diffs:
                    generated_paths_conditioned_on_length[layer_diff] = []
                    node_pairs_to_prompt = random.sample(list(set(list(eval_pairs_dict[layer_diff]))), cfg.batch_size)
                    tensors = [make_prompt(node_pair[0], node_pair[1], token_idx_map) for node_pair in node_pairs_to_prompt]
                    ...
                    with torch.no_grad():
                        with ctx:
                            for k in range(num_samples):
                                x,_ = next(dataloader)
                                # Generate a list of lists of sequences
                                generated_paths_conditioned_on_length[layer_diff].append(model.generate(input_tensor.to(device), max_new_tokens, temperature=temperature, top_k=top_k))

                    edge_accuracies_conditioned_on_length_r, does_end_at_targets_conditioned_on_length_r, path_lengths_conditioned_on_length_r = check_generated_path_accuracy(dag, generated_paths_conditioned_on_length[layer_diff], token_map)
                    edge_accuracies_conditioned_on_length[layer_diff] = edge_accuracies_conditioned_on_length_r
                    does_end_at_targets_conditioned_on_length[layer_diff] = does_end_at_targets_conditioned_on_length_r
                    path_lengths_conditioned_on_length[layer_diff] = path_lengths_conditioned_on_length_r 
                    edge_accuracies_conditioned_on_length[layer_diff][np.isnan(edge_accuracies_conditioned_on_length[layer_diff])] = 0
                ...
                wandb.log({
                    ...
                    "edge_accuracies_5": np.mean(edge_accuracies_conditioned_on_length['5']),
                    "edge_accuracies_6": np.mean(edge_accuracies_conditioned_on_length['6']),
                    "edge_accuracies_7": np.mean(edge_accuracies_conditioned_on_length['7']),
                    "edge_accuracies_8": np.mean(edge_accuracies_conditioned_on_length['8']),
                    "edge_accuracies_9": np.mean(edge_accuracies_conditioned_on_length['9']),
                    "does_end_at_target_5": np.mean(does_end_at_targets_conditioned_on_length['5']),
                    "does_end_at_target_6": np.mean(does_end_at_targets_conditioned_on_length['6']),
                    "does_end_at_target_7": np.mean(does_end_at_targets_conditioned_on_length['7']),
                    "does_end_at_target_8": np.mean(does_end_at_targets_conditioned_on_length['8']),
                    "does_end_at_target_9": np.mean(does_end_at_targets_conditioned_on_length['9']),
                    "path_lengths_5": np.mean(path_lengths_conditioned_on_length['5']),
                    "path_lengths_6": np.mean(path_lengths_conditioned_on_length['6']),
                    "path_lengths_7": np.mean(path_lengths_conditioned_on_length['7']),
                    "path_lengths_8": np.mean(path_lengths_conditioned_on_length['8']),
                    "path_lengths_9": np.mean(path_lengths_conditioned_on_length['9']),
                })
